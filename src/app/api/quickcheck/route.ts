...
async function checkBlacklists(domain: string) {
    ...
    // Check IP blacklists
    // HEURISTIC: For CDN IPs, skip ALL IP-based blacklists.
    // IP reputation is meaningless for shared infrastructure like Cloudflare.
    // Domain blacklists are still checked below.
    if (!isCDN) {
        ...
        console.info(`[CDN BYPASS] Skipping all IP blacklists for Cloudflare IP ${ip}`);
        ignoredCount = IP_BLACKLISTS.length; // Mark all as ignored for the note
    }
